...
namespace FamilyLabsCs
{
    ...
    class RvtCmd_FamilyCreateColumnVisibility : IExternalCommand
    {
        ...
        {
            double w = mmToFeet(600);
            double d = mmToFeet(600);
            double h = mmToFeet(4000);
            double t = mmToFeet(50);

            // define vertices 頂点の定義
            XYZ[] pts = new XYZ[]
            {
                new XYZ((-w/2) + t,  d/2,        0),
                new XYZ((-w/2) + t, (-d/2) + t,  0),
                new XYZ(w/2,        (-d/2) + t,  0)
            };

            XYZ ptH = new XYZ((-w / 2) + t, (-d / 2) + t, h);  // this is for vertical line.

            // (2) create a sketch plane 作業面の作成
            ReferencePlane pRefPlane = findElement(typeof(ReferencePlane), "参照面") as ReferencePlane;
            SketchPlane pSketchPlane = SketchPlane.Create(_rvtDoc, pRefPlane.GetPlane());

            XYZ normal = XYZ.BasisX;  // X軸?
            Plane pGeoPlaneH = Plane.CreateByNormalAndOrigin(normal, pts[1]);
            SketchPlane pSketchPlaneH = SketchPlane.Create(_rvtDoc, pGeoPlaneH);

            // (3) create line objects 線オブジェクトの作成
            ...
            ModelCurve pLineH = _rvtDoc.FamilyCreate.NewModelCurve(geomLineH, pSketchPlaneH);

            // set the visibilities of two lines to coarse only 可視性の設定
